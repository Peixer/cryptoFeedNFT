...
const Home: NextPage = () => {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-tr from-violet-600 to-violet-400 p-5">
          ...
          <div className="mb-5 w-full sm:flex sm:flex-auto">
            <form className="flex w-full " onSubmit={handleSubmit}>
              <input
                ...
                id="email"
                className="w-full rounded-md border-0 px-2.5 py-3 text-lg text-slate-200 shadow-md placeholder:text-gray-400 focus:outline-none  sm:leading-6"
              />
              <div className="ml-2">
                <WalletMultiButtonDynamic className="btn-ghost btn-sm  text-xl" />
              </div>
            </form>
            {/* <div className="mb-1 w-max sm:ml-2">
              <WalletMultiButtonDynamic className="btn-ghost btn-sm relative flex text-xl" />
            </div> */}
          </div>
        </div>
        {loading && (
        ...
        <div className="columns-1 gap-4 px-5 py-24 sm:columns-2 xl:columns-3 2xl:columns-4 ">
          {images.map(({ id, media_url, description }) => (
            <Link
              ...
              shallow
              className="after:content group relative m-auto mb-5 block w-full cursor-zoom-in after:pointer-events-none after:absolute after:inset-0 after:rounded-lg after:shadow-highlight">
              <Image
                alt="selected image"
